<template>
  <div>
    <gantt-row :name="title" expandable="1" v-model="showSlots" :act_depth="actDepth" :link="link">
    ...
    <div v-else-if="showSlots && issues.length > 0">
      ...
      <template v-for="issue in issues">
        ...
        <!-- issue is a node -->
        <gantt-group :act_depth="actDepth+1" v-else :key="'group' + issue.id" :title="issue.subject" :start_date="getStartEndForSubIssues(issue.sub_issues).startDate" :end_date="getStartEndForSubIssues(issue.sub_issues).endDate" :issues="issue.sub_issues" :link = "'issues/'+issue.id"/>
      </template>

      <!-- draw sub projects -->
      <template v-for="sub_project in sub_projects">
        <gantt-row :key="'project' + sub_project.id" :name="sub_project.name" expandable="1" v-model="showChildSlots[sub_project.id]" :act_depth="actDepth+1" :link="'projects/'+sub_project.name">
          <gantt-bar :start_date="start_date" :end_date="end_date" type="sum"/>
          <gantt-time-line :milestones="milestones"></gantt-time-line>
        </gantt-row>
        <div :key="'sub_issues' + sub_project.id" v-if="showChildSlots[sub_project.id] && sub_project.issues.length > 0">
          <template v-for="sub_project_issue in sub_project.issues">
            ...
            <gantt-group
              ...
              :link = "'issues/'+sub_project_issue.id"
            />
          </template>
        </div>
      </template>
...
export default {
  mixins: [formatter],
  name: "GanttGroup",
  props: ["title", "milestones", "start_date", "end_date", "issues", "sub_projects", "act_depth", "link"],
  data() {
    return {
